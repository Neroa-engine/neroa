import { buildWorkspaceName, splitList } from "./planning-core";
import type {
  GeneratedPlan,
  PlanningAnswers,
  ReviewAction,
...
import { inferProjectTemplate } from "@/lib/workspace/project-lanes";
  ...
  return [
    {
      ...
      status: "active",
      reason: "Neroa drives intake, planning, synthesis, routing, and execution guidance across the product."
    },
    {
      id: "atlas",
      name: "Atlas",
...
function buildRecommendedStack(primaryLaneId: LaneId, supportingLaneIds: LaneId[], answers: PlanningAnswers) {
  const stack = ["Neroa Workspace", "Neroa Execution Layer"];
  ...
  return [
    {
      ...
      items: [
        ...
        `Use Neroa to tighten the goal: ${answers.mainGoal || "create a measurable first outcome"}.`
      ]
    },
    {
  ...
  return teammates.map((teammate) => {
    if (teammate.id === "atlas" && shouldActivateAtlas) {
      return {
        ...
        reason: "Neroa recommends Atlas for deeper reasoning, research, and long-context analysis."
      };
    }
    ...
    if (teammate.id === "forge" && shouldActivateForge) {
      return {
        ...teammate,
        status: "recommended" as const,
        reason: "Neroa recommends Forge when the plan needs product, site, automation, or implementation help."
      };
    }
    ...
    if (teammate.id === "repolink" && shouldActivateRepo) {
      return {
        ...teammate,
        status: "recommended" as const,
        reason: "Neroa recommends RepoLink when repository or systems context will affect execution."
      };
    }
...
export function generatePlan(answers: PlanningAnswers): GeneratedPlan {
  ...
  return {
    ...
    projectTemplateId,
    overview: `${answers.idea} Neroa is framing this as a ${primaryLane.name.toLowerCase()} workspace with the right supporting lanes to carry the idea into execution.`,
    ...
    refinementNotes: supportingLanes.length > 0
      ? [`Neroa assigned supporting lanes: ${supportingLanes.map((lane) => lane.name).join(", ")}.`]
      : []
  };
}
...
export function createSynthesisMessage(plan: GeneratedPlan) {
  ...
  return `Perfect. Neroa has enough context to draft this as a ${primaryLane.name} workspace${supportingText}. Review the plan below, then refine it or open the workspace.`;
}
