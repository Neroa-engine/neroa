...
const businessWorkspaceProfile: WorkspaceIntakeProfile = {
  intro:
    "Narua is active in this Business NeuroEngine. Give me the business idea first and I will run a guided intake before I build the execution plan.",
  helperText:
    "Start intake with Narua. Tap the mic and speak naturally, then press Send when you are ready.",
  questionFields: [
    "businessType",
    "targetCustomer",
    "services",
...
const genericWorkspaceProfile: WorkspaceIntakeProfile = {
  intro:
    "Narua is active in this NeuroEngine. Start with the core idea and I will gather the minimum context needed before I shape the execution plan.",
  helperText:
    "Start intake with Narua. Tap the mic and speak naturally, then press Send when you are ready.",
  questionFields: [
    "businessType",
...
function buildPrompt(
  ...
  if (laneId === "business") {
    switch (field) {
      ...
      case "websiteNeeds":
        return "Do you need Narua to include website, domain, and brand setup in the plan?";
      case "idea":
  ...
  switch (field) {
    ...
    case "startupBudget":
      return "What budget does Narua need to plan around?";
    case "timeline":
      return "What timeline should Narua plan around?";
    case "geographicFocus":
      return "What market, territory, or user segment matters first?";
    case "websiteNeeds":
      return "Should Narua include website, domain, or brand setup in the plan?";
    case "idea":
...
export function createWorkspaceGeneratedSections(
  ...
  return {
    overview: [
      ...
      `Narua is keeping Business as the lead engine while Website, Marketing, and Operations support execution.`
    ],
    businessStrategy: [
...
export function createWorkspaceIntakeSynthesisMessage(
  ...
  return `Narua has enough context to draft the ${lane.name.toLowerCase()} execution system for ${answers.idea}. Review the generated strategy, budget, website setup, and roadmap, then refine the next action from there.`;
}
